...
from goflow.apptools.forms import DefaultAppForm, ContentTypeForm
from goflow.apptools.models import ImageButton, Icon, Image
...
def start_application(request, app_label=None, model_name=None, process_name=None, instance_label=None,
                      ...
                      form_class=None, formset=None, redirect='home', submit_name='action',
    ...
    if request.method == 'POST':
        ...
        if submit_value == ok_value and form.is_valid():
            ...
            if ob:
                if with_formset:
                    formset = formset(request.POST, request.FILES, instance=ob)
                    if formset.is_valid():
                        formset.save()

                priority = int(form.cleaned_data['priority'])
                ProcessInstance.objects.start(
                    process_name, request.user, ob, instance_label, priority=priority)
    ...
    ex = {'form': form, 'process_name': process_name, 'formset': formset,
